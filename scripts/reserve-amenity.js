...
function generateReservationTimes() {
  ...
  // Round down to the hour for start time
...
function makeReservation(courtId, courtName) {
  return new Promise((resolve, reject) => {
    ...
    var options = {
      ...
      'path': '/api/v1/my/occupants/131027/amenity-reservations/',
      'headers': {
        'accept': '*/*',
        'accept-language': 'en-GB,en;q=0.9,en-US;q=0.8,zh-CN;q=0.7,zh;q=0.6',
        'content-type': 'application/json',
        'origin': 'https://my.tfc.com',
        'priority': 'u=1, i',
        'referer': 'https://my.tfc.com/',
    ...
    var req = https.request(options, function (res) {
      ...
      res.on("end", function (chunk) {
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          resolve({ success: true, court: courtName, response: body.toString() });
        } else {
          console.log(`❌ ${courtName} reservation failed!`);
          resolve({ success: false, court: courtName, response: body.toString(), status: res.statusCode });
        }
      });
...
async function reserveAmenity() {
  ...
  try {
    // Try TENNIS_COURT_1 first
    const court1Result = await makeReservation(TENNIS_COURT_1 || "8", "Tennis Court 1");
    ...
    // Try TENNIS_COURT_2 if first attempt failed
    const court2Result = await makeReservation(TENNIS_COURT_2 || "10", "Tennis Court 2");
